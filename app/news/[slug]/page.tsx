import { basysArticleBodyBySlug } from "@/lib/news/basysArticleBodies";
import {
    formatNewsDate,
    getAllBasysSlugsFromNews,
    getBasysNewsBySlug,
...
export function generateStaticParams() {
    return getAllBasysSlugsFromNews().map((slug) => ({ slug }));
}
...
export async function generateMetadata({
    ...
    const item = getBasysNewsBySlug(slug);
    if (!item) {
        return { title: "News | basys.ai" };
    }
    return {
        title: `${item.title} | basys.ai`,
        description:
            item.description ??
            `Basys.ai news: ${item.title.slice(0, 140)}${item.title.length > 140 ? "…" : ""}`,
    };
}

export default async function BasysNewsArticlePage({ params }: PageProps) {
    const { slug } = await params;
    const item = getBasysNewsBySlug(slug);
    const fullBody = basysArticleBodyBySlug[slug];
