      ...
      <div className="mx-auto max-w-7xl px-8">
        <div className="mx-auto mb-20 max-w-2xl text-center">
          ...
          <p className="text-lg text-on-surface-variant">
            Healthcare infrastructure requires more than just intelligence-it demands
            absolute governance and interoperability.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
          {valueProps.map((valueProp) => (
            <article
              key={valueProp.title}
              className="flex flex-col gap-6 rounded-[2rem] border border-outline-variant/30 bg-surface-container-lowest p-8"
            >
              ...
              <div className="space-y-3">
                <h3 className="text-xl font-semibold">{valueProp.title}</h3>
                <p className="text-sm text-on-surface-variant">{valueProp.description}</p>
              </div>
            </article>
          ))}
        </div>
